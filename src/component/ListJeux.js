...
import { Typography, Box, TextField, ListItem, Select, ListItemText, List, MenuItem, FormControl, InputLabel, ListItemButton} from '@mui/material';
...
export default function ListJeux() {
  ...
  const handleSearchByName = (event) => {
    ...
    if (searchName !== '') {
    axios.get(`http://localhost:3000/jeu/byName/${searchName}`)
      .then(response => {
  ...
  const handleTypeChange = (event) => {
    ...
    axios.get(`http://localhost:3000/jeu/byType/${searchType}`)
        .then(response => {
          setJeux(response.data);
        })
        .catch(error => {
  ...
  React.useEffect(() => {
    axios.get('http://localhost:3000/zone/')
      .then(response => {
  ...
  const handleZoneChange = (event) => {
    ...
    axios.get(`http://localhost:3000/zone/${searchZone}`)
        .then(response => {
  ...
  return (
    <Box sx={{display: 'flex', flexDirection: 'column', alignItems: 'center', mb:10}}>
      ...
      <RadioGroup
        ...
        sx={{display: 'flex', flexDirection: 'row', justifyContent: 'space-between', mt:5}}
      >
        {['Par nom', 'Par type', 'Par zone'].map((value) => (
          <Radio
            ...
            sx={{
              ...
              fontWeight: 'lg',
              fontSize: 'large',
              backgroundColor: selectedValue === value ? '#F88F52' : 'background.paper',
      ...
      {selectedValue === 'Par nom' && (
        <Sheet sx={{ p: 2, borderRadius: 'md', boxShadow: '2', bgcolor: 'background.body' }}>
          <div>
            <TextField label="Recherche par nom" variant="outlined" onChange={handleSearchByName} value={searchValue} />
          </div>
        </Sheet>
      )}
        ...
        <Sheet sx={{ p: 2, borderRadius: 'md', boxShadow: '2', bgcolor: 'background.body' }}>
          <div>
            <FormControl sx={{ m: 1, minWidth: 300 }} >
              ...
              >
                {zones.map(zone => (
                  <MenuItem key={zone._id} value={zone._id}>{zone.nomZone}</MenuItem>
                ))}
              </Select>
            </FormControl>
          </div>
      ...
      <List>
        {jeux.map((jeu) => (
          <ListItem key={jeu._id} component="div" disablePadding sx={{bgcolor: 'background.paper', boxShadow: 2, borderRadius: 2, m:2}}>
            <ListItemButton>
